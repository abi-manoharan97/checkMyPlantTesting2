...
def upload_file():
    if request.method == "POST":
        ...
        resp = requests.post("https://predict-pjpj3nqmga-ue.a.run.app", files={"file":file})
        print(resp.json())
    ...
    return render_template(
        "classify.html", imagefile=img_data, label=resp.json().get('prediction'), prob=resp.json().get('prediction') 
    )
